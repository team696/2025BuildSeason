...
public class Swerve extends TunerSwerveDrivetrain implements Subsystem {
  ...
  private SysIdRoutine m_sysIdRoutineToApply = m_sysIdRoutineRotation;//m_sysIdRoutineTranslation;
  ...
  public static SwerveRequest.ApplyRobotSpeeds rcDriveReq = new SwerveRequest.ApplyRobotSpeeds()
    ...
    BaseCam.acceptEstimate estimate = (Estimate) -> {
      ...
      if (Estimate.distToTag < 1) {
        ...
        setVisionMeasurementStdDevs(
            VecBuilder.fill(Estimate.ambiguity * Math.pow(Estimate.distToTag, 2)*0.01,
                Estimate.ambiguity * Math.pow(Estimate.distToTag, 2)*0.01,
                Estimate.ambiguity * Math.pow(Estimate.distToTag, 2)*0.01));
      }
      return true;
    };
