...
public class ClimberIntake extends SubsystemBase {
  ...
  TalonFX master=new TalonFX(BotConstants.ClimberIntake.masterID, BotConstants.rioBus), slave=new TalonFX(BotConstants.ClimberIntake.slaveID, BotConstants.rioBus);
  ...
  public Command Position(DoubleSupplier position){
    return this.startEnd(()->master.setControl(positionRequest.withPosition(position.getAsDouble())), ()->master.set(0));
  }
  ...
  public void periodic() {
    ...
    BackupLogger.addToQueue("ClimberIntake/masterVelocityRps", masterVelocitySignal.refresh().getValue().in(RotationsPerSecond));
    ...
    BackupLogger.addToQueue("ClimberIntake/slaveVelocityRps", slaveVelocitySignal.refresh().getValue().in(RotationsPerSecond));
